...
def exception_handler(exc: Exception, context: dict[str, Any]) -> Response | None:
    ...
    ``ProblemDetailsMixin`` and ``@problem_details``.
    ...
    ``WWW-Authenticate`` / ``Retry-After`` are set. An error status outside the catalogue gets the
    ``about:blank`` type that RFC 9457 reserves for problems with no extra semantics.
